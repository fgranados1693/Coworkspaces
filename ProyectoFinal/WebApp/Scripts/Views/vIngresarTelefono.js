...
function vIngresarTelefono() {
	...
	this.Create = async function () {
        try {
            ...
            localStorage.setItem('newUserTelefono', data['telefono']);
            ...
            //this.ctrlActions.PostToAPI(this.service, telefonoData);
            ...
            if (this.ValidateForm()) {
                if (await this.ValidateTelUnique(data['telefono'])) {
                    ...
                    if (response.status != 200) {
                        ...
                        ctrlActions.ShowMessage('I', "Enviamos un código a tu número de teléfono");
                        setTimeout(function () { window.location.href = "vVerificarTelefono"; }, 3000);
                    }
    ...
    this.ValidateForm = function () {
        ...
        if (/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/.test(tel)) {
            ...
            ctrlActions.ShowMessage('E', "El formato del número de teléfono es inválido");
            document.getElementById('txtTelefono').classList.remove("is-valid");
            document.getElementById('txtTelefono').classList.add("is-invalid");
            return false;
        }
    ...
    this.ValidateTelUnique = async function (tel) {
        try {
            ...
            for (i = 0; i < json['Data'].length; i++) {
                ...
                if (u['Telefono'] == tel) {
                    ctrlActions.ShowMessage('E', "Ya existe un usuario con este número de teléfono");
                    document.getElementById('txtTelefono').classList.remove("is-valid");
                    document.getElementById('txtTelefono').classList.add("is-invalid");
                    return false;
                }
